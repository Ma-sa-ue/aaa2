...
for epoch in range(n_epochs):
    ...
    for start, end in zip(
        ...
        if np.mod(iterations, 500) == 0:
            ...
            save_visualization(generated_samples, (14,14), save_path='./vis10_8/sample_'+str(iterations/500)+'.jpg')
        if np.mod(iterations, 500) == 0:    
            np.save("./vis10_8/array1_"+str(iterations/500),np.array(p_gen_list))
            np.save("./vis10_8/array2_"+str(iterations/500),np.array(p_val_list))
            np.save("./vis10_8/array3_"+str(iterations/500),np.array(loss_list))
        iterations += 1
